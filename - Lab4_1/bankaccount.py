class BankAccount:
    ...
    def __init__(self, name, account_type="saving", balance=0):
        ...
        if self.type.lower() == "saving":
            type_code = BankAccount.__type_saving
            BankAccount.last_saving_number += 1
            running_num = BankAccount.last_saving_number
        else:
            type_code = BankAccount.__type_loan
            BankAccount.last_loan_number += 1
            running_num = BankAccount.last_loan_number
            
        self.account_number = f"{BankAccount.branch_number}-{type_code}-{running_num}"
